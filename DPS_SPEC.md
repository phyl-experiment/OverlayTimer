...
- 메뉴 열릴 때마다 현재 창 상태를 반영해 텍스트 자동 갱신
- 최소화 시 작업 표시줄 버튼은 유지됨
...
key1(uint) -> 스킬 이름 매핑 테이블. 숫자 키를 문자열로 표현.
파일이 없거나 키가 없으면 숫자 ID 그대로 표시(폴백).
스킬명 표기 규칙은 `직업-스킬` 형식을 사용한다. (예: `대검-평타`)
