...
public sealed class PacketBuffStart
{
    ...
    public ulong SessionOrActorKey { get; private set; } // offset 0x08
    public uint BuffKey { get; private set; }            // offset 0x10
    public float Value1 { get; private set; }            // offset 0x14
    ...
    {
        // 최소: userId(8) + key64(8) + buffKey(4) + float(4) + int(4) + int(4) = 32
        ...
        p.SessionOrActorKey = ReadU64(content, ref pos); // 0x08
        p.BuffKey = ReadU32(content, ref pos);           // 0x10
        p.Value1 = ReadF32(content, ref pos);            // 0x14
